...
def image_from_data(
    ...
    **kwargs
    ...
    """Takes a numpy array and transforms it into an image in the [0, 1] range. Applies border wrapping, resizing and normalization.
    ...
    wrapping: string, default: 'both'
                                    What side of the image to add a wrapped border.  Possible values are the available in WrappingType Enum.
    wrap_size: int, default: 1
                                    Size of the wrapping applied to the data. IMPORTANT: this is done before resizing, so keep in mind that the wrapping will be upscaled.
    normalization_method: string, default: 'histogram'
                                    Method used for normalization. Possible values are the available in NormalizationMethod Enum.
    interpolation_method: cv2.INTER_METHOD, default: cv2.INTER_LINEAR
                                    Interpolation method used for resizing of the data. Uses values directly from OpenCV.
    normalization_first: bool
                                    The 'histogram' and 'CLAHE' normalization methods require the data to be put in a [0, 255] range.
